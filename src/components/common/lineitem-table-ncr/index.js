...
const LineItemtableNCR = ({ data, handleRefreshData, bay }) => {
        ...
        () => [
            {
                ...
                Cell: ({ data, row }) => {
                    return data[row.id].file_link ? <a href={data[row.id].file_link} target="_blank" rel="noreferrer">{data[row.id].product_name}</a> : data[row.id].product_name
                }
            },
            {
                Header: 'Status',
...
export default LineItemtableNCR