import { forwardRef, useCallback, useEffect, useMemo, useRef, useState, useContext } from 'react';
...
import { getAllColours, getJobsBay, getLocation, getUsers, jobLabel, updateBayJobStatus } from '../../../../server';
import {
...
import SelectJobStatus from '../../../common/select';
...
const SchedulingBay = ({ isMenuExpanded }) => {
    ...
    const columns = useMemo(() => {
        ...
        return tableColumns;
    }, [handleRefreshData, selectedItems]);
    ...
    const getKanbanBayData = useCallback(
        ...
        [bay, filterEndDate, filterStartDate]
    );
    ...
    useEffect(() => {
        getJobsBayData();
        getColours();
    }, [bay, isGettingData, getJobsBayData]);
    ...
    const handleUpdateStatusModal = async () => {
        ...
        for (let index = 0; index < selectedItems.length; index++) {
            await updateBayJobStatus({ [bay + '_status']: selectedStatus }, selectedItems[index]).catch(() => {
                hasError = true;
            });
        }
        ...
        <>
            ...
            >
                <div style={{ marginLeft: 40 }}>
                    <PageTitle title={`${bay.charAt(0).toUpperCase() + bay.slice(1)} Bay`} />
                </div>
                    ...
                    <div className='loading-container'>
                        ...
                        kanbanError == null ? (
                                <div className={styles.kanbanComponentWrapper}>
                                    <DragDropContext
                                        onDragEnd={(result) =>
                                            onDragEnd(result, kanbanData, setKanbanData, (value) => jobCardRef.current.calculateBaysDate(value), bay)
                                        }
                                    >
