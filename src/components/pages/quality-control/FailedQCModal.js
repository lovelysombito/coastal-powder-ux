...
const FailedQCModal = ({ showModal, setShowModal, selectedData, setSelectedData, sigCanvas, handleRefreshData}) => {
    ...
    const handleFailStatus = () => {
        ...
        selectedData.data.forEach((item) => {
            if (item.checked) {
                selectedIdsInString += `${item.id ?? item.lineItem},`;
            } else {
                selectedPassIdsInString += `${item.id ?? item.lineItem},`;
            }
        });
    ...
    const updateQCJobStatus = async (form_data) => {
        await updateQCJob(form_data, selectedData.jobId, { header: { 'Content-Type': 'multipart/form-data', 'Access-Control-Allow-Origin': '*' } })
        .then((res) => {
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                <Modal.Body>
                    ...
                    <div className={styles.jobIdModal}>
                        {
                            selectedData.invoiceNumber == null ? 
                                <>
                            ...
                            selectedData.data.map((item, i) => (
                                <div key={i} className='form-check'>
                                    <input
                                        ...
                                        name={item.id}
                                    />
                                    <label htmlFor={item.lineItem} className='form-check-label'>
                                        {item.lineItem}
                                    </label>
                                </div>
                            ))}
